...
- **`TECH_STACK_DECISIONS.md`** (v1.2) — Approved technology decision record: database
  ...
  (`TECH-MOB-001`, FROZEN 2026-08-28: Flutter, Android + iOS from day one, Hive/Drift offline
...
- **`CROSS_MODULE_PRINCIPLES.md`** (`ARCH-CROSS-001`, added 2026-08-26/27, v1.1.0) — Project-wide
...
- **`IMPLEMENTATION_DEPENDENCY_ORDER.md`** (`SOL-ARCH-008` / `IMPLEMENTATION-TIER-001`) —
  ...
  Property added to Tier 6). Status: FROZEN as of 2026-08-28 per the document's own header and
  §44 — **note:** its closing §79 status summary was not updated by that freeze commit and
  still reads DRAFT/v0.1.0/21-modules, contradicting the header; needs its own follow-up fix.
- **`FK_DEPENDENCY_GRAPH.md`** (`SOL-ARCH-009`, added 2026-08-28) — Physical FK dependency
  ...
  Status: FROZEN.
- **`DDL_CREATION_ORDER.md`** (`SOL-ARCH-010`, added 2026-08-28) — The exact numbered
...
- **`PROGRAMMES_EVENTS_CROSS_MODULE_REVIEW.md`** (`SOL-EVT-006`, now v1.1.0) — Final
  compatibility review for the Programme & Events module (Module #21) against every other
  module; concludes "architecturally justified," no hard conflicts. **Updated 2026-08-28: status
  header now reads FROZEN** (up from DRAFT v0.1.0) — its closing §80 "Reconciliation Closure"
  ...
  document" / "Module #21 = PROPOSED" — leftover from before the 2026-08-28 update, not removed;
  the header/§80 supersede them in intent but the internal inconsistency wasn't cleaned up.
- **`PROGRAMMES_EVENTS_RECONCILIATION_DECISIONS.md`** (`SOL-EVT-007`, added 2026-08-28, v1.0.0,
  ...
  and freezes two new sub-principles — `P&E-ARCH-001` (common Registration capability, financial
  transactions still Finance-owned) and `P&E-ARCH-002` (Event Session is optional and
  organiser-defined). Grew the P&E candidate-table set from 5 to 7 (`event_day` and
  `event_registration` added). The tables themselves remain CANDIDATE, not frozen DDL, pending
