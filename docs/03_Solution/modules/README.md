...
As of 2026-08-28, every module below has a complete Solution-level doc set except `family`
...
(`SOL-EVT-007`) all 7 of its reconciliation gates against other modules are now closed (its
candidate-table set also grew from 5 to 7); the module itself still awaits formal freeze.
**`assets_property/` (22nd module, added 2026-08-27 — Module #22)** is tagged
`v1.0.0 DRAFT — SOURCE ALIGNED` like most others. Six modules (`person`, `family`, `governance`,
`attendance`, `authentication`, `administration`) each gained a new `_lifecycle.md` document in
the same session that added the 21st module, shifting their existing business-rules/table-design
file numbers down one slot — check each module's own `README.md` for its current exact file list
before referencing a filename by number. A cross-module architecture pass on 2026-08-26/27
...
| `person/` | v1.0.0, SOURCE ALIGNED — **1 table** (`person` only — `document_master` reassigned to Foundation, `DOC-ARCH-001`, 2026-08-26), 5 files | `database/ddl/03_person/` implements `person`/`person_address` (as `person_code`, docs say `person_id`) |
...
| `programmes_events/` | v0.1.0, DRAFT — NOT FROZEN (Module #21) — **7 candidate common tables** (`programme_type`, `event`, `event_day`, `event_session`, `event_registration`, `event_location`, `event_history`), none frozen DDL, but all 7 cross-module reconciliation gates now closed (`SOL-EVT-007`, 2026-08-28); Programme Type → Event Instance two-level model | no `backend/programmes_events/` app |
| `assets_property/` | v1.0.0, DRAFT — SOURCE ALIGNED (Module #22, added 2026-08-27) — 7 tables (`property`, `asset`, `custodianship`, `property_statutory_record`, `maintenance_record`, `property_document`, `asset_document`); 74 business rules (`AP-001`–`AP-074`) | no `backend/assets_property/` app |
