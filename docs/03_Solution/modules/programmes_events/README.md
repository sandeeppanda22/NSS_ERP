...
`event_registration`, `event_location`, `event_history` (`event_day` and `event_registration`
added 2026-08-28 by the reconciliation-gates closure below — grew from the original 5).
`financial_scope` (Finance-owned), attendance entities, and UPBS/Kishor/Sevak event extensions
are explicitly **not** among these 7 and remain domain-owned. Note: this document's own header
(`Version: 0.1.0` / `DRAFT — TABLE DESIGN`) was not bumped when its content was updated for the
reconciliation closure — the header undersells how settled the table list now is.
...
written. `PROGRAMMES_EVENTS_CROSS_MODULE_REVIEW.md` (`SOL-EVT-006`, now v1.1.0 **FROZEN**) —
...
**Updated 2026-08-28 — reconciliation gates closed:**
...
`MODULE_DEPENDENCY_MAP.md` (`SOL-ARCH-007`) lists this as one of 22 modules now (Assets &
Property was added as Module #22, 2026-08-27), "ARCHITECTURALLY JUSTIFIED / RECONCILIATION
...
now **FROZEN** as `IMPLEMENTATION-TIER-001`, up from PROPOSED).
...
2026-08-28) but still not frozen DDL** · SQL Implementation Not Started · no
